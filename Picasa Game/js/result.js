let imposterName = window.localStorage.getItem("imposter");
let playersVoted = document.querySelector(".content .container .result > div:first-of-type h3")
let theImposter = document.querySelector(".content .container .result > div:nth-of-type(2) h3")
let winLose = document.querySelector(".content .container .result .win-lose p");
let winLoseH3 = document.querySelector(".content .container .result .win-lose h3");
let winLosePlayersIcon = document.querySelector(".content .container .result .win-lose .icon:first-of-type");
let winLoseImposterIcon = document.querySelector(".content .container .result .win-lose .icon:nth-of-type(2)");
let playAgainBtn = document.querySelector(".content .container:has(.result) button")
...
console.log(winLosePlayersIcon)
console.log(winLoseImposterIcon)
...
window.onload = ()=>{
    ...
    winLosePlayersIcon.style.display = "none";
    winLoseImposterIcon.style.display = "none";
    theImposter.textContent = imposterName;
    theImposter.style.cssText = `
            font-size: ${300 / theImposter.textContent.length}px;
        `
    if(window.localStorage.getItem("maxVotesArray")){
        var maxVotesArray = window.localStorage.getItem("maxVotesArray").split(",");
    }
    console.log(maxVotesArray)
    if(maxVotesArray.length === 1){
        playersVoted.textContent = maxVotesArray[0];
        playersVoted.style.cssText = `
            font-size: ${300 / playersVoted.textContent.length}px;
        `
        if(maxVotesArray[0] === imposterName){
            winLose.textContent = "players";
            winLose.style.color = '#0ec00e';
            winLoseH3.textContent = "WIN!";
            winLosePlayersIcon.style.display = "block";
            winLosePlayersIcon.style.color = "#0ec00e";
            winLoseImposterIcon.style.display = "none";
        }else{
            winLose.textContent = "imposter";
            winLose.style.color = 'rgb(235 0 0)';
            winLoseH3.textContent = "WINS!";
            winLosePlayersIcon.style.display = "none";
            winLoseImposterIcon.style.display = "block";
            winLoseImposterIcon.style.color = "rgb(235 0 0)";
        }
    }else{
        playersVoted.textContent = "a tie";
        console.log("LOSE")
        winLose.textContent = "imposter";
        winLose.style.color = 'rgb(235 0 0)';
        winLoseH3.textContent = "WINS!";
